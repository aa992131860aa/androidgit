...
import com.daming.entity.Paichj;
...
public class yemianer extends Activity implements OnClickListener {
    ...
    private void realPro(View v,int pTimeNo,int pProNo){
    	if(proNo==-1&&timeNo==-1){
    		//Toast.makeText(yemianer.this, "请选择生产时段", Toast.LENGTH_SHORT).show();
    		if(pProNo==0){
			...
			try {
				...
				if(resEnd<0||(dateFormat.parse(nowsStr).getTime()<0&&dateFormat.parse(endTime).getTime()>0)){
					//Toast.makeText(yemianer.this, "生产时段", Toast.LENGTH_SHORT).show();
//					if(timeNo>=0){
...
//					}
               ...
               //  proNo = 0;
                // textPeroidList1.get(temp).setBackgroundColor(Color.parseColor("#449D44"));
					...
					Toast.makeText(yemianer.this, "还没到达排产时间", Toast.LENGTH_SHORT).show();
				}
...
//					}
	...
	public void onClick(View v) {
		...
		case R.id.main_lly_equipment:
			...
			hujiao(equipmentStart, equipmentEnd, v, 3,hjjlh3);
			break;
		...
		case R.id.btn_img1:
			...
			intent.setClass(yemianer.this, chengyuanxinxi.class);
			if (getPaic != null) {
		...
		case R.id.btn_img2:
			...
			intent2.setClass(yemianer.this, chengyuanxinxi.class);
			if (getPaic2 != null) {
		...
		case R.id.btn_img3:
			...
			// Toast.makeText(yemianer.this, getPaic.getFzr().getRydm(),
			// Toast.LENGTH_SHORT).show();
			Intent intent3 = new Intent();
			intent3.setClass(yemianer.this, chengyuanxinxi.class);
		...
		case R.id.btn_img4:
			...
			intent4.setClass(yemianer.this, chengyuanxinxi.class);
		...
		case R.id.btn_img5:
			...
			intent5.setClass(yemianer.this, chengyuanxinxi.class);
		...
		case R.id.btn_chanxian1:
			...
			intent_cx.setClass(yemianer.this, chanpinjiaohu.class);
		...
		case R.id.btn_chanxian2:
			...
			intent_cx2.setClass(yemianer.this, chanpinjiaohu.class);
		...
		protected void onPostExecute(List<ConfirmChanxdm> result) {
			...
			if (result != null && result.size() > 0) {
				if (result.get(0).getResult().equals("1")) {
					...
					if(flag==1) {
						ShareUtils.setHjjlh1(yemianer.this, hjjlh);
					}
					if(flag==2) {
						ShareUtils.setHjjlh2(yemianer.this, hjjlh);
					}
					if(flag==3) {
						ShareUtils.setHjjlh3(yemianer.this, hjjlh);
					}
					if(flag==4) {
						ShareUtils.setHjjlh4(yemianer.this, hjjlh);
					}
					if(flag==5) {
						ShareUtils.setHjjlh5(yemianer.this, hjjlh);
					}if(flag==6) {
						ShareUtils.setHjjlh6(yemianer.this, hjjlh);
					}if(flag==7) {
						ShareUtils.setHjjlh7(yemianer.this, hjjlh);
					}if(flag==8) {
						ShareUtils.setHjjlh8(yemianer.this, hjjlh);
					}



				} else {
					Toast.makeText(yemianer.this, "呼叫失败", Toast.LENGTH_SHORT).show();
					if (flag == 1) {
		...
		protected void onPostExecute(List<ConfirmChanxdm> result) {
			...
			if (result != null && result.size() > 0) {
				if (result.get(0).getResult().equals("1")) {
					...
					Toast.makeText(yemianer.this, "呼叫返回成功", Toast.LENGTH_SHORT).show();
					// dialog.create().cancel();
					if(flag==1){
						ShareUtils.setHjjlh1(yemianer.this,"");
					}
					if(flag==2){
						ShareUtils.setHjjlh2(yemianer.this,"");
					}
					if(flag==3){
						ShareUtils.setHjjlh3(yemianer.this,"");
					}
					if(flag==4){
						ShareUtils.setHjjlh4(yemianer.this,"");
					}
					if(flag==5){
						ShareUtils.setHjjlh5(yemianer.this,"");
					}
					if(flag==6){
						ShareUtils.setHjjlh6(yemianer.this,"");
					}
					if(flag==7){
						ShareUtils.setHjjlh7(yemianer.this,"");
					}
					if(flag==8){
						ShareUtils.setHjjlh8(yemianer.this,"");
					}
					alertDialog.dismiss();
				} else {
					Toast.makeText(yemianer.this, "呼叫返回失败", Toast.LENGTH_SHORT).show();
					if (flag == 1) {
...
//						};
//					}.start();
					 Toast.makeText(yemianer.this, "录入成功", Toast.LENGTH_SHORT).show();
					}
					else if(proNo==1){
...
//							}.start();
					...
					Toast.makeText(yemianer.this, result.getMsg(), Toast.LENGTH_SHORT).show();
					
				}
			}else{
				Toast.makeText(yemianer.this, "录入失败", Toast.LENGTH_SHORT).show();
			}
		...
		protected void onPostExecute(Shouqjs result) {
			...
			if (result != null ) {
				if (result.getResult().equals("1")) {
					if(proNo==-1){
						if(pProNo==0){
							Toast.makeText(yemianer.this, "解锁成功", Toast.LENGTH_SHORT).show();
					...
					Toast.makeText(yemianer.this, "解锁失败", Toast.LENGTH_SHORT).show();
					alertDialog.dismiss();
				}
			}
	...
	private void realProDialog(final View v, final int id, final int flag) {
		...
		View view = LayoutInflater.from(yemianer.this).inflate(
		...
		dialog.setPositiveButton("确定", new DialogInterface.OnClickListener() {
			...
			public void onClick(DialogInterface arg0, int arg1) {
				...
				if (name.equals("")) {
					Toast.makeText(yemianer.this, "请输入产品数量", Toast.LENGTH_SHORT).show();
					...
					String pcclId = "";
					if (flag == 0) {
						pcclId = mGetPaics.get(0).getCpList().get(0)
								.getPcclList().get(id).getPcclid();
					} else if (flag == 1) {
						pcclId = mGetPaics.get(0).getCpList().get(1)
								.getPcclList().get(id).getPcclid();
					}
					if (!pcclId.equals("")) {
	...
	private void unlockPro(final View v, final int pProNo, final int pTimeNo) {
		AlertDialog.Builder dialog = new AlertDialog.Builder(yemianer.this);
		dialog.setTitle("请扫描解锁条码");


		final View view = LayoutInflater.from(yemianer.this).inflate(
		...
		dialog.setPositiveButton("确定", new DialogInterface.OnClickListener() {
			...
			public void onClick(DialogInterface arg0, int arg1) {
				UnlockProTask proTask = new UnlockProTask(v, pProNo,pTimeNo, hjjl_edt.getText()
						.toString());
				proTask.execute();
				// Toast.makeText(yemianer.this, "非专业人员请勿操作", Toast.LENGTH_SHORT).show();
			}
		});
		...
		TextWatcher textWatcher = new TextWatcher() {
			...
			public void onTextChanged(CharSequence s, int start, int before,
					int count) {
				// TODO Auto-generated method stub
				// Toast.makeText(yemianer.this, "changed", Toast.LENGTH_SHORT).show();
			...
			public void beforeTextChanged(CharSequence s, int start, int count,
					int after) {
				// TODO Auto-generated method stub
				// Toast.makeText(yemianer.this, "before", Toast.LENGTH_SHORT).show();
			...
			public void afterTextChanged(Editable s) {
				...
				if (hjjl_edt.getText().toString().length() >= 5) {
					// Toast.makeText(yemianer.this, "after", Toast.LENGTH_SHORT).show();
					// dialog.create().dismiss();
		...
		public void handleMessage(android.os.Message msg) {
			...
			case Constant.REQUEST_DATA:
				...
				case Constant.REQUEST_ERROR:
					Toast.makeText(yemianer.this,"服务连接错误,请重试",Toast.LENGTH_SHORT).show();
					finish();
					break;
			/**
			 * 产能信息处理
			...
			case Constant.REQUEST_SAVEPAICWCSL_DATA:
				...
				//Toast.makeText(yemianer.this, "提交", Toast.LENGTH_SHORT).show();

				break;
			case Constant.REQUEST_SAVEPAICWCSL_ERROR:
				View v1 = (View) msg.obj;
				v1.setVisibility(View.VISIBLE);
				Toast.makeText(yemianer.this, "提交完成产量失败", Toast.LENGTH_SHORT).show();
				break;
	...
	private void initData() {
        ...
        try{
			Activity activity = yemianer.this;
			while (activity.getParent() != null) {
				activity = activity.getParent();
			}

			// 用户通过AlertDialog选择生产时段和班次
		final AlertDialog.Builder builder = new AlertDialog.Builder(
				activity);
		mSingleChoiceID = -1;
		builder.setTitle("请选择班次与日期");
		if (mGetPaicLists != null && mGetPaicLists.size() > 0) {
			String items []= new String[mGetPaicLists.size()];
			List<PaicList> getPaicLists = new ArrayList<>();
              SimpleDateFormat sdf = new SimpleDateFormat("yyyy-MM-dd HH:mm");
			  String nowStr = sdf.format(new Date());

			//nowStr = "2015-04-01 08:00";
			Date now;
			Calendar calendar = Calendar.getInstance();
			calendar.add(Calendar.DAY_OF_MONTH, -1);
			now = calendar.getTime();
			//Toast.makeText(yemianer.this,sdf.format(now),Toast.LENGTH_SHORT).show();
			int count = 0;
			try {
				//now = sdf.parse(nowStr);

				for (int i = 0; i < mGetPaicLists.size(); i++) {
                 Date paicDate =   sdf.parse(mGetPaicLists.get(i).getScrq()+" 07:58");

					if(now.getTime()<=paicDate.getTime()){
						getPaicLists.add(mGetPaicLists.get(i));
					items[count++] = "生产日期：" + mGetPaicLists.get(i).getScrq()
							+ "                       " + "班次："
							+ mGetPaicLists.get(i).getBcdm();
						mGetPaicLists.get(i);
				}
				}
             ...
             if(count>0) {
				 final String[] mItems = new String[count];
				 for (int i = 0; i < count; i++) {
					 mItems[i] = items[i];
				 }
              mGetPaicLists = getPaicLists;
					...
					new DialogInterface.OnClickListener() {
						public void onClick(DialogInterface dialog,
							...
							chanxian.setText("产线代码(PL Code)：" + mUsername
									+ "        ");
							main_lly.setVisibility(View.VISIBLE);
							dialog.dismiss();

									 //加载getPaic数据

							new GetPaicDataTask().execute();
			...
			builder.create().show();
				 ...
				 Toast.makeText(this,"没有排产,请排产后登录",Toast.LENGTH_SHORT).show();
				 finish();
			 }
		}
		}catch (Exception e){
			Toast.makeText(this,"排产出差",Toast.LENGTH_SHORT).show();
			finish();
		}
	}
    ...
    public class SavePaicTask extends AsyncTask<Void , Void, List<ConfirmChanxdm>>{
		...
		protected void onPostExecute(List<ConfirmChanxdm> result) {
			...
			if(result!=null){
				...
				boolean isSave = savePaicList.get(0).getResult().equals("1");
				//Toast.makeText(yemianer.this, isSave+"", Toast.LENGTH_SHORT).show();
				//sw.getView().setVisibility(View.VISIBLE);
				int number1 = 0;
				int number2 = 0;
				if (isSave) {
				...
				Toast.makeText(yemianer.this, "保存失败", Toast.LENGTH_SHORT).show();
			}
		}
...
//			mHandler.sendMessage(msg);
    ...
    private class GetPaicDataTask extends AsyncTask<Void,Void,List<GetPaic>>{
		...
		@Override
		protected void onPostExecute(List<GetPaic> getPaics) {
			super.onPostExecute(getPaics);
			if(getPaics!=null){
				//mHandler.sendEmptyMessage(Constant.REQUEST_GETLIST_DATA);
				dealProduct();
			}else{
				//mHandler.sendEmptyMessage(Constant.REQUEST_GETLIST_REEOR);
				Toast.makeText(yemianer.this, "获取产品失败" , Toast.LENGTH_SHORT).show();
			}
	...
	private void dealProduct(){
		...
		if (mGetPaics.get(0).getCpList() != null
				&& mGetPaics.get(0).getCpList().size() > 2) {
			main_btn_next.setBackgroundColor(Color.parseColor("#449D44"));
		}else{
		...
		if (mGetPaics != null && mGetPaics.size() > 0
			...
			// 设置生产时段1
			List<Pccl> pcclList1 = mGetPaics.get(0).getCpList().get(0)
					.getPcclList();
			String scgx = mGetPaics.get(0).getCpList().get(0).getScgx();
			if (scgx == null) {
				scgx = "";
			}
			btn_chanxian1.setText(mGetPaics.get(0).getCpList().get(0)
					.getCpdm()
					+ "  " + scgx);
			cpdm1 = mGetPaics.get(0).getCpList().get(0).getCpdm();
			// 修改
			if(mGetPaics.get(0).getCpList().get(0).getYxcc().equals("0")){
			...
			btn_chanxian1.setOnClickListener(yemianer.this);
			int real1 = 0;
			int real2 = 0;
			double sumOne1;
			...
			if (mGetPaics.get(0).getCpList().size() >= 2) {
				// 设置生产时段1
								...
								public void onClick(View v) {
									...
									try {
										...
										if((resStart>=0&&resEnd>=0)||(dateFormat.parse(startTime).getTime()>0&&dateFormat.parse(endTime).getTime()<0)){
											...
											//Toast.makeText(yemianer.this, pcclid+"", Toast.LENGTH_SHORT).show();
											proRealNumber  = Integer.parseInt(pcclPlanList1.get(temp).getWcsl() != null
													? pcclPlanList1.get(temp).getWcsl() : "0");
											textPeroidList1.get(temp).setBackgroundColor(Color.parseColor("#449D44"));
										}else{
											Toast.makeText(yemianer.this, "请选择相应生产时段", Toast.LENGTH_SHORT).show();
										}
			...
			if (mGetPaics.get(0).getCpList().size() >= 2) {
				...
				List<Pccl> pcclList2 = mGetPaics.get(0).getCpList()
						.get(1).getPcclList();
				String scgx2 = mGetPaics.get(0).getCpList().get(1)
						.getScgx();
				if (scgx2 == null) {
					scgx2 = "";
				}
				...
				btn_chanxian2.setText(mGetPaics.get(0).getCpList()
						.get(1).getCpdm()
						+ "  " + scgx2);
				cpdm2 = mGetPaics.get(0).getCpList().get(1).getCpdm();
				if(mGetPaics.get(0).getCpList().get(1).getYxcc().equals("0")){
				...
				btn_chanxian2.setOnClickListener(yemianer.this);
			...
			if (mGetPaics.get(0).getFzr() != null
				...
				Picasso.with(yemianer.this)
						.load(mGetPaics.get(0).getFzr().getSrc())
						.into(btn_img1);
				btn_img1.setOnClickListener(yemianer.this);
			}
			if (mGetPaics.get(0).getJyy() != null
					&& !mGetPaics.get(0).getJyy().getSrc().equals("")) {

				btn_img2.setOnClickListener(yemianer.this);
				Picasso.with(yemianer.this)
			...
			if (mGetPaics.get(0).getCztList() != null
				...
				for (int i = 0; i < size; i++) {
					...
					imageList.get(i).setOnClickListener(yemianer.this);
	...
	private List<GetPaic> loadGetPaicData() {
		...
		String data = RequestDataUtil.callMethod(mUsername, mUserpwd, param,
				method_name, ip);
		test = pcid;
		//Log.e("yemianer1:", data);
		if (!data.equals("")&&data.contains("root")) {
			data = RequestDataUtil.splieRequestXml(data);
			try {
				mGetPaics = RequestDataUtil.parseGetPaicXml(data);
...
//				}
	...
	public CountDownTimer timer2 = new CountDownTimer(Integer.MAX_VALUE,180000) {
		...
		public void onTick(long millisUntilFinished) {
			// TODO Auto-generated method stub
			//Toast.makeText(yemianer.this, tTimeNo+"gg", Toast.LENGTH_SHORT).show();
			...
			if(tTimeNo!=-1){
			 ...
			 String times [] = time.split("-");
					...
					String startTime =times[0];
					String endTime = times[1];
					//Toast.makeText(yemianer.this, endTime, Toast.LENGTH_SHORT).show();
					try {

						long resEnd = dateFormat.parse(endTime)
								.getTime()
								- dateFormat.parse(nowsStr).getTime();
						//Toast.makeText(yemianer.this, tTimeNo+"gg"+dateFormat.parse(nowsStr).getTime()+"gg"+dateFormat.parse(endTime).getTime(), Toast.LENGTH_SHORT).show();
	...
	protected void onStart() {
		...
		ShareUtils.setIp(yemianer.this, ip);
		if (mUsername != null && mUserpwd != null) {
			main_rel.setVisibility(View.VISIBLE);
			main_lly.setVisibility(View.GONE);
			new LoadPaicListTask(mUsername,mUserpwd).execute();
		}
	...
	private void addListener() {
		chanxian.setOnClickListener(new OnClickListener() {
			public void onClick(View v) {
				Intent intent = new Intent();
				intent.setClass(yemianer.this, chanxianjiaohu.class);
				intent.putExtra("scrq", scrq);
				intent.putExtra("username", mUsername);
				intent.putExtra("userpwd", mUserpwd);
				intent.putExtra("ip", ip);
		...
		btn_img_daming.setOnClickListener(new OnClickListener() {
			public void onClick(View v) {
				Intent intent = new Intent();
				intent.setClass(yemianer.this, TongZiActivity.class);
				intent.putExtra("username", mUsername);
				intent.putExtra("userpwd", mUserpwd);
				intent.putExtra("ip", ip);
				startActivity(intent);
		...
		main_btn_next.setOnClickListener(new OnClickListener() {
			...
			public void onClick(View view) {
				if (mGetPaics.get(0).getCpList() != null
						&& mGetPaics.get(0).getCpList().size() > 2) {
					//yemianer2
					Intent intent = new Intent(yemianer.this, yemianer2.class);
					intent.putExtra("username1", mUsername);
					intent.putExtra("userpwd1", mUserpwd);
					intent.putExtra("mSingleChoiceID", mSingleChoiceID);
					intent.putExtra("mGetPaicLists",(Serializable) mGetPaicLists);
					intent.putExtra("mGetPaics", (Serializable) mGetPaics);
					intent.putExtra("ip1", ip);
					startActivity(intent);
				}
			}
		});
	...
	private void init() {
		...
		ip = ShareUtils.getIp(yemianer.this);
			...
			public void afterTextChanged(Editable s) {
               ...
               // Toast.makeText(yemianer.this,"请选择生产时段",Toast.LENGTH_SHORT).show();
            	 // index = 0;
            	   ...
            	   Toast.makeText(yemianer.this, "现在的输入:"+content, Toast.LENGTH_SHORT).show();
				   main_edt_pro.setText("");
				   index = -1;
